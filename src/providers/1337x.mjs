...
function parseMainPageHTML(html) {
    ...
    $('.table-list tbody tr').each((index, element) => {
        ...
        const seeders = $(element).find('.coll-2.seeds').text();
        ...
        torrents.push({ title, link: `https://www.1377x.to${link}`, seeders, leeches, uploadDate, size });
    });
    return torrents;
}
...
async function scrape1337x(query, categories, pages = 1) {
    const categoryURL = {
        ...
        'TV Shows': `https://www.1377x.to/category-search/${query}/TV/`,
